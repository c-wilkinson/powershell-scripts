...
WITH src AS (
    ...
    FROM STRING_SPLIT(REPLACE(@raw, CHAR(13), ''), CHAR(10), 1)
)
...
SELECT f.first_name, l.surname
INTO #people
FROM #first_name f
CROSS JOIN #surname l
ORDER BY NEWID()
...
WITH u AS (
  SELECT TOP ($(USERS))
    first_name,
    surname,
    usernamebase = CONCAT(LEFT(first_name, 1), surname),
    usernamern   = ROW_NUMBER() OVER (
             PARTITION BY CONCAT(LEFT(first_name, 1), surname)
             ORDER BY (SELECT 1)
           ) - 1
  FROM #people
  ORDER BY NEWID()
...
INSERT INTO dbo.Users (Username, FirstName, Surname, Email)
SELECT 
  Username  = CONCAT(usernamebase, NULLIF(CAST(usernamern AS varchar(10)), '0')),
  FirstName = first_name,
  Surname = surname,
  Email = CONCAT(usernamebase, NULLIF(CAST(usernamern AS varchar(10)), '0'), '@', domain_name)
FROM u
CROSS APPLY (
  SELECT TOP 1 domain_name
  FROM email_domains
  ORDER BY CHECKSUM(NEWID(), first_name, surname) & 2147483647
) d;
